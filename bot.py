...
🚀 PRODUCTION TELEGRAM VPN BOT (AUTO PAY + AUTO ISSUE)
...
await call.message.answer(
    "💰 Пополнение\n\nПосле оплаты бот автоматически зачислит баланс.",
    reply_markup=kb
)
...
try:
    # DonationAlerts payload
    username = data["data"]["username"]
    amount = int(data["data"]["amount"])

    user_id = int(username)  # ⚠ ВАЖНО: пользователь должен донатить своим ID

    users_balance[user_id] = users_balance.get(user_id, 0) + amount

    # авто выдача если >= 100
